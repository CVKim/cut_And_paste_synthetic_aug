...
def extract_defect_features(mask_path):
    ...
    return np.array([cx, cy, np.log1p(area)])  # np.log1p를 사용하여 면적의 로그 값을 사용

# 특성 벡터 추출 및 스케일링
features = np.array([extract_defect_features(mask) for _, mask in tqdm(paired_files, desc='Extracting Features')])
scaler = StandardScaler()
features_scaled = scaler.fit_transform(features)  # 특성 스케일링

# KMeans 클러스터링
n_clusters = 5  # 클러스터 수 설정, 실험을 통해 최적의 수를 찾아야 할 수 있음
...
plt.title('Clustering Results with Scaled Features')

graph_path = os.path.join(output_dir, 'clustering_results.png')
